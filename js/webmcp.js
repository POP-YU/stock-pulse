  ...
  async function registerTools() {
    ...
    results.push(await register(context, {
      ...
      execute: async () => {
        ...
        if (!snapshot.refresh.ok) {
          return {
            ok: false,
            stale: snapshot.refresh.stale,
            error: snapshot.refresh.error || 'Quote refresh did not complete',
            ranked: [],
            count: 0,
...
, description: 'A-share, HK code, or US ticker such as 600519, 00700, or AAPL' },
      }, ['symbol']),
      annotations: { readOnlyHint: true, untrustedContentHint: true },
      execute: async (args) => {
        const quote = await app().fetchQuote(boundedSymbol(args));
...
, description: 'A-share, HK code, or US ticker to add' },
      }, ['symbol']),
      annotations: { readOnlyHint: false, destructiveHint: false, untrustedContentHint: true },
      execute: async (args) => {
        const result = await app().addToWatchlist(boundedSymbol(args));
